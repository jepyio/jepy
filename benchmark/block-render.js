...
bench('repeating block', () => repeatingBlockTemplate.render({
    items
}));

const repeatingBlockWithAliasTemplate = new jepy.Template('#{arrayItems:item}\${item}#{/arrayItems}');
let arrayItems = [];
for (let i = 0; i < 1000; i++) {
    arrayItems.push('Lorem ipsum dolor sit amet');
}
bench('repeating block with alias', () => repeatingBlockWithAliasTemplate.render({
    arrayItems
}));
...
bench('indented block', () => indentedBlock.render({
...
bench('complex template', () => complexTemplate.render({
